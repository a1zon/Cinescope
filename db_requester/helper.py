...
class DBHelper:
    ...
    def get_movie_by_name(self, name: str):
        ...
        return self.db_session.query(FilmDBModel).filter(FilmDBModel.name == name).first()

    def get_movie_by_id(self, movie_id: int):
        """Получает фильм по названию"""
        return self.db_session.query(FilmDBModel).filter(FilmDBModel.id == movie_id).first()
